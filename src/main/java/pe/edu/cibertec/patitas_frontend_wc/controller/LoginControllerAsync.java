...
public class LoginControllerAsync {
    ...
    public Mono<LogoutResponseDTO> cerrarSesion(@RequestBody LogoutRequestDTO logoutRequest) {
        try {
            return webClientAutenticacion.post()
                    .uri("/logout")
                    .body(Mono.just(logoutRequest), LogoutRequestDTO.class)
                    .retrieve()
                    .bodyToMono(LogoutResponseDTO.class);
        } catch (Exception exception) {
            System.out.println(exception.getMessage());
            return Mono.just(new LogoutResponseDTO(false, null, "Ocurrió un error durante el cierre de sesión"));
        }
    }
